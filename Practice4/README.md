...
Чем отличаются типы памяти в CUDA и в каких случаях их использовать?

Ответ: Глобальная память большая, общая для всех потоков, но медленная. Shared память маленькая, быстрая, для потоков одного блока. Регистры супербыстрые, но у каждого потока свои. Используем глобальную для больших массивов, shared когда потокам блока нужны одни данные, регистры для временных переменных.

Как использование разделяемой памяти влияет на производительность?

Ответ: Если данные несколько раз нужны потокам блока, сначала загружаем их в shared память, а потом используем. Это намного быстрее, чем каждый раз читать из глобальной памяти.

Доступ и как его обеспечить?

Ответ: Для быстрого доступа нужно, чтобы потоки обращались к памяти подряд. Если они читают случайно или через большие скачки, GPU сильно тормозит.

Какие сложности возникают при работе с большим объёмом данных на GPU?

Ответ: GPU имеет ограниченную память, и большие массивы могут её переполнить. Кроме того, при большом объёме важно правильно организовать доступ, иначе чтение/запись замедлит программу.

Почему важно минимизировать доступ к глобальной памяти?

Ответ: Потому что глобальная память медленная. Если часто её читать или писать, ядро будет тратить больше времени на ожидание данных, а не на вычисления.

Как использовать профилирование для анализа производительности CUDA-программ?

Ответ: Запускаем программу через профайлер (например, Nsight). Смотрим, сколько времени тратится на память, вычисления и синхронизацию. Так можно понять, что тормозит и что оптимизировать в первую очередь.
